...
Dim BI
If rsTk("Frostmengde") >= 35000 Or Request("Tr") = "R" Then
	BI = AvrundOpp(P/1.1)*1.1/1.1
End If

Dim stkSal
If IsNumeric(Ms) Then stkSal = AvrundOpp(Ms/0.7)
...
<FORM NAME="Materialliste" METHOD="Post" ACTION="SendEpost.asp" onSubmit="return Verifiser()">
  <TABLE WIDTH="606">
    ...
    <TR>
      ...
      <B>Jackon Ringmurselement:</B><br></TD>
    </TR>
    <TR>
      <TD ALIGN="Left" colspan="2">
...
Sub SettInnRingmurselement()
	...
	If stkSal > 0 Then
		...
		strHTML = strHTML & "<TR VALIGN=Top><TD>" & rsVare("NOBB") & "</TD><TD>"
		strHTML = strHTML & rsVare("Varenavn") & "</TD><TD>" & (stkSal) & " " & rsVare("Beskrivelse") & " = " & (stkSal * 0.75) & " m</TD><TD ALIGN=Right>"
...
Sub SettInnGulvisolasjon()
	...
	strHTML = strHTML & "<TR VALIGN=Top><TD>&nbsp;</TD><TD>Samlet tykkelse gulvisolasjon "
	strHTML = strHTML & rsGI("Total") & "mm</TD><TD>&nbsp;</TD><TD ALIGN=Right>"
